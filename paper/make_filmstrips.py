...
def strip(video_path: str) -> np.ndarray:
...
def main():
    top = strip(f"{VIDEOS}/cotrain_goalT0_drawer_SUCCESS_ep0.mp4")
    bottom = strip(f"{VIDEOS}/250k_goalT0_drawer_FAIL_ep0.mp4")
    divider = np.full((DIVIDER_PX, top.shape[1], 3), 255, dtype=np.uint8)
    cv2.imwrite("figs/drawer_compare.png", np.vstack([top, divider, bottom]))

    cv2.imwrite("figs/lib10_dualobj_fail.png", strip(f"{VIDEOS}/cotrain_lib10T7_putBOTH_FAIL_ep0.mp4"))
    print("wrote figs/drawer_compare.png, figs/lib10_dualobj_fail.png")
