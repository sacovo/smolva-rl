#!/usr/bin/env python3
"""SLURM array worker: one eval cell (checkpoint x suite x cfg weight) per task.

Copy of the file running on the cluster as scripts/rat_aug.py (job 179462).
The rat_* family (rat_snapflow_v2..v7.py, rat_noki_nodrop.py, ...) differ only
in the three constants below; edit CHECKPOINTS/SUITES/CFG_SCALES and the
matching #SBATCH --array bound in the sub_*.sh wrapper for a new sweep.
"""
import os
import sys
import json
...
CHECKPOINTS = [('aug_n500','outputs/libero_aug/recap_model/migrated')]

SUITES = ['libero_spatial', 'libero_goal', 'libero_10', 'libero_object']
CFG_SCALES = [0.0, 0.5, 1.0, 1.5, 2.0]
N_EPISODES = 50  # 5 episodes per task * 10 tasks = 50 episodes total
